...
{ config, lib, pkgs, ... }:
{
    options.jnc.setRootPasswordOnFirstBoot = lib.options.mkOption {
        type = lib.types.bool;
        description = ''
            Whether or not to prompt for a root password on first boot.

            This option allows you to install NixOS on a machine without
            setting its root password. The first time that new
            installation of NixOS is booted, it will ask the user to
            enter a new root password.
        '';
    };
    config = lib.modules.mkIf config.jnc.setRootPasswordOnFirstBoot {
        systemd.services.ensure-root-has-password = {
            ...
            script = let
                resholvedScript = pkgs.resholve.writeScript
                    "ensure-root-has-password-resholved-script"
                    {
                        inputs = with pkgs; [
                            coreutils
                            dbus
                        ...
                        execer = [
                            ...
                            # this PR [1] is merged and released.
                            #
                            # editorconfig-checker-disable
                        ...
                        function set_systemd_show_status {
                            # I used --print-reply here in order to
                            # force dbus-send to wait for a reply.
                            # Waiting for a reply helps ensure that
                            # systemd has finished setting ShowStatus by
                        ...
                        function restore_original_systemd_show_status {
                            # This is supposed to work according to this
                            # commit message [1].
                            #
                            # editorconfig-checker-disable
                        ...
                        if [ ! -e "$marker" ]
                        then
                            # If we don’t turn off systemd’s ShowStatus
                            # option, then we’ll run into
                            # this problem [1].
                            #
                            # editorconfig-checker-disable
                            ...
                            do
                                ...
                                then
                                    ...
                                    >&2 echo \
                                        There was an error setting the \
            ...
            serviceConfig = {
                ...
                StandardError = "tty";
                Type = "oneshot";
                # TODO: If this works, remove the commented out line.
